...
- **ADR-005 re-export propagation** — barrel/re-export chains resolved iteratively with cycle detection.
...
The `duplicates/` module implements **suffix array + LCP based clone detection** (*verified* against [`mod.rs`](https://raw.githubusercontent.com/fallow-rs/fallow/main/crates/core/src/duplicates/mod.rs)). Pipeline: `tokenize_file()` → `normalize_and_hash_resolved()` → `CloneDetector` builds suffix array + LCP arrays to find repeated token sequences in **O(n log n)** (no quadratic pairwise comparison) → `families::group_into_families()` clusters instances → mirrored-directory detection → suppression + `min_occurrences` filtering → deterministic sort. Four normalization modes (*verified* in `normalize.rs`): **Strict** (exact tokens / Type-1), **Mild** (default, basic structural normalization), **Weak** (blinds strings), **Semantic** (blinds identifiers + literals / Type-2). Thresholds: `min_tokens`, `min_lines`, `skip_local`.

### 2.6 Plugin system (122 plugins)

Plugins implement a `Plugin` **trait** (*partly verified*: the trait and methods are real, but "pure static data" overgeneralizes — the trait is split into static `&'static` defaults *plus* dynamic AST-driven resolution). Each plugin declares: `entry_patterns()`/`entry_pattern_rules()`/`entry_point_role()`; `config_patterns()`/`resolve_config()` (parses config files via Oxc into dynamic facts)/`package_json_config_key()`; `used_exports()`/`used_export_rules()`; `path_aliases()`/`auto_imports()`. Dynamic resolution emits a `PluginResult { entry_patterns, used_exports, referenced_dependencies, path_aliases, provided_dependencies, ... }`. End-users can author custom framework plugins declaratively in JSONC/JSON/TOML with no code.

### 2.7 Command set and output contracts

Commands: `fallow` (full), `audit`, `review`, `decision-surface`, `health`, `dead-code`, `dupes`, `security`, `trace <symbol>`, `watch`, `fix --dry-run`, `inspect`, `impact`, `coverage` (paid), `license`, `init`. Output formats (`cli/report/`): **human, json, sarif, compact, markdown** (README also lists CodeClimate); the `types` crate owns the serde contract. Severity: `error` (CI fail, default) / `warn` / `off`. Eight finding categories: Quality Score (0–100), PR Risk verdict, Hotspots, Duplication families, Architecture, Dependency Hygiene, Cleanup, Runtime Intelligence (paid). Suppression: `// fallow-ignore-next-line <rule>`, `// fallow-ignore-file`, JSDoc `@public/@internal/@beta/@alpha/@expected-unused`, and declarative rule packs (`banned-call`/`banned-import`/`banned-effect`).

### 2.8 Caching and git-diff incrementalism

Persistent cache `.fallow/cache/`; extraction AST cache (`.fallow/cache.bin`) capped by `FALLOW_CACHE_MAX_SIZE` (default 256 MB, LRU eviction); encoded with **bitcode v0.6** (*verified*: `Cargo.lock` pins `bitcode = "0.6.9"`). Audit incrementalism (*verified* against [`audit.rs`](https://raw.githubusercontent.com/fallow-rs/fallow/main/crates/cli/src/audit.rs)): `git::get_changed_files()` + `auto_detect_base_ref()`; when gate = `NewOnly` and the current tree isn't reusable, it spins an isolated git worktree at the base via `BaseWorktree::create()`, runs the same passes, captures an `AuditKeySnapshot`. Cache key = **xxHash3-64** of (cache version, CLI version, base SHA, config hash, changed-files list, production settings, workspace config, baseline paths). Attribution: `AuditAttribution` + `count_introduced()` partition findings into introduced vs inherited; `compute_introduced_verdict()` gates only new issues. The base-snapshot cache is capped at 16 MiB (`MAX_AUDIT_BASE_SNAPSHOT_CACHE_SIZE`).
...
- **MCP** (stdio): `inspect_target` (combined trace+dead-code+dupes+complexity), `code_execute` (bounded read-only sandbox), `security_candidates`.
- **Agent Skill**: version-matched guidance shipped in npm.
- **Free/OSS (MIT):** all static analysis, CLI/LSP/MCP, all output formats, CI templates, 122 plugins, config/suppression/baselines (*verified*).
- **Paid Fallow Runtime:** runtime coverage (V8 + Istanbul), `health --runtime-coverage`, hot-path/cold-code evidence, cloud aggregation, runtime-weighted scoring, gated by offline Ed25519 (EdDSA, algorithm-pinned) JWT (*verified*, with the nuance that static *coverage-gap* analysis is free; only *runtime* coverage and some closed-source normalization are paid).

### 2.10 Performance

Dead-code vs knip v6 (M5, 10c/32GB): Preact 244f 74ms vs 2.01s (27×); Fastify 286f 64ms vs 205ms (3.2×); TanStack/query 901f 560ms vs 1.04s (1.9×); Svelte 3,337f 611ms vs 632ms (~equal); TypeScript 38,146f 2.22s vs 736ms (knip 3× faster on the very largest repo). Knip v5/v6 fail on next.js/vite/vue-core where fallow succeeds. Standalone dupes vs jscpd: jscpd is 1.4×–14.7× faster — fallow's dupe value is *integration into the unified audit*, not winning standalone.
