...
Steps:
1. Run `mollify graph` (emits Graphviz DOT to stdout; `graph` is a CLI command — for architecture *findings* use the mollify MCP `mollify_arch` tool). Add `--path <dir>` if a subproject was specified. Add `--mermaid` to emit a Mermaid diagram instead of DOT.
